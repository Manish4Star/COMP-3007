...
#|
Changed all the lets to lambda on following lines: 
216-224,230-238,227-239,326-328, 
332-340,299-301,348-356,359-364,432-436 
|#
      ...
      ((lambda (first rest )
        (if (cond-else-clause? first)
            ...
            (make-if (cond-predicate first)
                     (sequence->exp (cond-actions first))
                     (expand-clauses rest)))) (car clauses) (cdr clauses))))
...
(define (lookup-variable-value var env)
  (define (env-loop env)
    ...
    (if (eq? env the-empty-environment)
        ...
        ((lambda (frame )
          (scan (frame-variables frame)
                (frame-values frame))) (first-frame env))))
  (env-loop env))
